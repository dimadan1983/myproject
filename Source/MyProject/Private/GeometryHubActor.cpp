...
{
	UE_LOG(LogGeometryHub, Warning, TEXT("Actor1 name: %s, Color: %s"), *Name, *Color.ToString());
}

void AGeometryHubActor::OnTimerFinished(AActor* Actor)
{
	if (!Actor) return;
	UE_LOG(LogGeometryHub, Error, TEXT("Timed1 finished: %s"), *Actor->GetName());

	ABaseGeomeryActor* Geometry = Cast<ABaseGeomeryActor>(Actor);
	if (!Geometry) return;
